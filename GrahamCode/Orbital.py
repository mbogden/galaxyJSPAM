...
#	print orbit[-1,:]
	...
	print "ascending node: " + str(ascNode)
	ascNode2 = ascNode*pReal[8]
	
	argPer = math.acos( np.dot( ascNode, rmin[:3]/LA.norm(rmin[:3]) ) )*180.0/np.pi
	print "arg of periapsis: " + str(argPer)
	
	refDir  = np.array( [ cos(pt)*cos(pp), cos(pt)*sin(pp), -sin(pt) ] )
	refDir2 = refDir*pReal[8]
	print "reference dir (x): " + str(refDir)
	...
	# end
	print "long. of asc. node: " + str(longAN)
...
#	p[2] = 1000
	...
	call("./basic_run_orb -g " + paramStr + " > SolveMetro.out", shell=True)
	
	fileList = sorted( glob.glob("a_1.0*") )
	
	# include 000?????
	
	orbit = []
	for f in fileList:
		orbit.append( np.loadtxt( f ) )
	# end
