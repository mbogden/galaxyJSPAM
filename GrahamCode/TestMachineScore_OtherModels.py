...
#	fileID = sys.argv[1]
#	pTest  = int(sys.argv[2])
	
#	fileID = 'z'
	pTest = 3
	
	labels = [ 'z', 'vx', 'vy', 'vz', 'mr', 'mt', 'rp', 'rs', 'pp', 'ps', 'tp', 'ts' ]
	fileID = labels[pTest-2]
	...
	direct   = "../TestFitness_MultipleGalaxies/Target_" + zooInd + "/"
	
	fileBase = "RV_" + zooInd
	fileP = fileBase + "_P_" + fileID + ".pk"
	fileT = fileBase + "_T.pk"
	fileV = fileBase + "_V.pk"
	fileM = fileBase + "_M_" + fileID + ".pk"
	fileU = fileBase + "_U_" + fileID + ".pk"
	
	pVals = pickle.load( open(direct + fileP, "rb") )
	RVt   = pickle.load( open(direct + fileT, "rb") )
	RVv   = pickle.load( open(direct + fileV, "rb") )
	RVm   = pickle.load( open(direct + fileM, "rb") )
	RVu   = pickle.load( open(direct + fileU, "rb") )
...
#	mASt = (nGen-1)/2
	...
	T = BinField( nBin, RVt,  bound )
	V = BinField( nBin, RVv, bound )
	...
	# read RV files
	for i in range(nGen):
		M[i,:,:] = BinField( nBin, RVm[i,:,:], bound )
		U[i,:,:] = BinField( nBin, RVu[i,:,:], bound )
	# end
...
#			score[:,j,i] = (2*tmScore[:,j]*tmScore[:,0]/(tmScore[:,j]+tmScore[:,0]))*muScore2[:,i]
#			score[:,j,i] = 2*tmScore[:,j]*muScore2[:,i]/(tmScore[:,j]+muScore2[:,i])
		# end
	# end
...
#	ls = 1
	...
	if(   toPlot == 1 ):
		fig, axes = plt.subplots(nrows=nScr+1, ncols=nScr+1)
#		axes = axes.flatten()
#		fig.set_size_inches(15,10)
		fig.set_size_inches(12,8)
		
		functions = [ 'RMSE', 'OvrAll', 'OvrU', 'Corr', 'bCorr', 'M1', 'M2', 'M3' ]
		
		for i in range(nScr):
#			axes[0,i+1].plot( pVals[:,pTest], tmScore[:,i], 'bo',  markersize=ms )
			axes[0,i+1].plot( pVals[:,pTest], tmScore[:,i], 'b', linewidth=ls )
			axes[0,i+1].set_title( functions[i] )
			axes[0,i+1].set_xlim( pVals[0,pTest], pVals[-1,pTest] )
			axes[0,i+1].set_ylim( 0, 1 )
			
			if( plotUnp ):
#				axes[i+1,0].plot( pVals[:,pTest], muScore2[:,i], 'ko',  markersize=ms )
				axes[i+1,0].plot( pVals[:,pTest], muScore2[:,i], 'k', linewidth=ls )
			else:
#				axes[i+1,0].plot( pVals[:,pTest], tmScore[:,i], 'bo',  markersize=ms )
				axes[i+1,0].plot( pVals[:,pTest], tmScore[:,i], 'b--', linewidth=ls )
			# end
			axes[i+1,0].set_title( functions[i] )
			axes[i+1,0].set_xlim( pVals[0,pTest], pVals[-1,pTest] )
			axes[i+1,0].set_ylim( 0, 1 )
			
			if( plotUnp ):

#				axes[i+1,0].plot( pVals[:,pTest], muScore[:,i], 'go',  markersize=ms )
				axes[i+1,0].plot( pVals[:,pTest], muScore[:,i], 'g', linewidth=ls )
				axes[i+1,0].plot( pVals[:,pTest], muScore[mASt,i]*np.ones(nGen), 'k', linewidth=ls )
			# end
		# end
		
		for i in range(nScr):
			for j in range(nScr):
#				axes[j+1,i+1].plot( pVals[:,pTest], score[:,i,j], 'ro',  markersize=ms )
				axes[j+1,i+1].plot( pVals[:,pTest], score[:,i,j], 'r--', linewidth=ls )
				axes[j+1,i+1].set_xlim( pVals[0,pTest], pVals[-1,pTest] )
				axes[j+1,i+1].set_ylim( 0, 1 )
			# end
		# end
