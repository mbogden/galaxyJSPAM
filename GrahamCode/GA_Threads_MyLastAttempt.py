...
#			x_start[j] = np.random.uniform(xLim[j,0],xLim[j,1])
	...
	chain, error = GA( nProc, x_start, p, xLim, j_sig, nGen, nDim, 0, progress, choice, burn, beta, mixAmp, mixProb, toMix, nPop )
...
#		print thr.current_thread().name + " GRRRRRR"
		return 0
	# end
	...
	time.sleep( np.random.random(1)[0]*0.01 )
	
#	print thr.current_thread().name + " in function"
...
def evalPop( nProc, nPop, popSol, choice, p ):
	
	#############################
	# Issues:
	# random hangups (join doesnt work is process is still alive (shouldnt it just wait?)
	# weird race condition causes queue to be popped even when nothing is there, had to ad hoc it in "f" function
	#############################
	...
	for i in range(nProc):
		ts.append( thr.Thread( target=f, args=(qS, qE, p, choice,) ) )
		ts[i].start()
#		print ts[i].name + " " + str(ts[i].is_alive())
	# end
...
#				print ts[i].name# + " " + str(ts[i].is_alive())
				ts[i].join()
#				print ts[i].name# + " " + str(ts[i].is_alive())
				ts[i] = thr.Thread( target=f, args=(qS, qE, p, choice,) )
				ts[i].start()
#				print ts[i].name + " " + str(ts[i].is_alive())
			# end
		# end
...
def GA( nProc, start, p, xLim, jump_sigma, nGen, nDim, tid, progress, choice, burn, beta, mixAmp, mixProb, toMix, nPop ):
...
#	print "step: init"
	...
	popErr = evalPop( nProc, nPop, popSol, choice, p )
