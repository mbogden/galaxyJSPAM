...
#	Ms       = pickle.load( open("models_"    + fileInd + ".txt", "rb") )
	elements = pickle.load( open("elements_"  + fileInd + ".txt", "rb") )
	orbits   = pickle.load( open("orbits_"  + fileInd + ".txt", "rb") )
...
#						cbar = fig.colorbar( II, ax=axes[jj,ii] )
		...
		nPts = len(orbits[0,0,:,0])
		
		"""
		for i in range(nPts):
#			ax.scatter( orbits[:,:,i,0].flatten(), orbits[:,:,i,1].flatten(), orbits[:,:,i,2].flatten(), c=scores[:,:].flatten(), cmap='jet',s=10 )
			
			x = orbits[:,:,i,0].flatten()
			y = orbits[:,:,i,1].flatten()
			z = orbits[:,:,i,2].flatten()
			II = ax.scatter( x[inds], y[inds], z[inds], c=c[inds], cmap='jet',s=10 )
			II.set_clim([0,1])
		# end
		"""
		...
		c = []
		for i in range(nPts):
			x.append( orbits[:,:,i,0].flatten() )
			y.append( orbits[:,:,i,1].flatten() )
			z.append( orbits[:,:,i,2].flatten() )
			c.append( scores.flatten() )
		# end
		...
		II = ax.scatter( x[inds][0], y[inds][0], z[inds][0], c=c[inds][0], cmap='jet',s=10, vmin=0, vmax=1 )
		II.set_clim([0,1])
#		cbar = fig.colorbar( II, ax=ax )
		
		ax.plot( orbitR1[:,0], orbitR1[:,1], orbitR1[:,2], 'k', linewidth=5 )
		ax.plot( orbitR2[:,0], orbitR2[:,1], orbitR2[:,2], 'm', linewidth=5 )
		...
		for i in range(nGen):
			for j in range(nPop):
				ax.plot( orbits[i,j,:,0], orbits[i,j,:,1], orbits[i,j,:,2], 'g' )
			# end
		# end
		"""
#		ax.plot( [0],[0],[0], 'rX', markersize=14 )
#		ax.plot( [pReal[0]],[pReal[1]],[pReal[2]], 'bX', markersize=14 )
	# end	
