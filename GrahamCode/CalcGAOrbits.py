...
#	orbit = solveOrb( chain[0,0,:], nParam )
	
	orbits = np.zeros((nGen,nPop,nKeep,6))
	for i in range(nGen):
		for j in range(nPop):
			orbit = solveOrb( chain[i,j,:], nParam )
			nStep, xxx = orbit.shape
			inds1 = map( int, np.linspace(0,nStep-1,nKeep) )
			orbits[i,j,:,:] = orbit[inds1,:]
		# end
		print str(i+1) + "/" + str(nGen)
	# end
	
	pickle.dump( orbits,   open("orbits_" + fileInd + ".txt", "wb") )
