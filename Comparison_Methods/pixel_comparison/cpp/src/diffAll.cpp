        ...
        if (runGood){
            ...
            for(unsigned int iImg=0; iImg < myRun.images.size() ; iImg++ ){
                ...
                if (imgGood){
                    ...
                    scoreFile << myRun.sdssName <<','<< myRun.runName  <<','<< targetName  <<','<< myRun.images[iImg].name  <<','<< myRun.images[iImg].paramName  <<','<< "absDiff_v3"<<',' <<score<<',';
                    if (hScoreFound)
...
//  Function does a rudimentary average of pixel brighteness across an image between 0 and 1.
//  Unused calculation of standard deviation
double rateDiff(const Mat &m1){

    unsigned long long int max, min, var1, var2, N, val;
    double score;
