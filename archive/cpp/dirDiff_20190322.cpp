 ...
 * Purpose: Single, straight forward code that compares
 *          two images by doing a direct difference in pixel
 *          brightness and calculating the average brightness. 
 */
...
//  Global Variables
string dirPath,  targetName, temp;
...
bool processMainDir();
bool processTarget(string, string, imgClass &);
...
// ./diff.out  main_directory  target_image target_image_info.txt  score_file.csv 
int main(int argc, char *argv[]){
    cout << endl;

    printf("HI!  MATTHEW HERE!  Program starting!\n");

    string runPath = argv[1];
    string targetPath = argv[2];
    string targetInfoPath = argv[3];
    string scorePath = argv[4];
    ...
    bool targetGood = processTarget(targetPath, targetInfoPath, target);
    if ( ! targetGood ){   //  Processes target data and info data.
        printf("\nTarget image or information file failed to process.\n");
        printf("Target image: '%c'\n",targetPath.c_str());
        printf("Target info file: '%c'\n\n",targetInfoPath.c_str());
        return 0;
    }
    ...
    if (runGood){
        ...
        for(unsigned int iImg=0; iImg < myRun.images.size() ; iImg++ ){
            ...
            if (imgGood){
                ...
                scoreFile << myRun.sdssName <<','<< myRun.runName  <<','<< targetPath <<','<< myRun.images[iImg].name  <<','<< myRun.images[iImg].paramName  <<','<< "absDiff_v2"<<',' <<score<<',';
                scoreFile << endl;
      ...
      printf("Failed to process run folder '%c'.\nExiting...",runPath.c_str());
    }
...
bool processTarget(string targetPath, string targetInfoPath, imgClass & target ){
